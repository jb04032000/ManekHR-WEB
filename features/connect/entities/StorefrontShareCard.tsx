...
const QR_DOWNLOAD_ID = 'cn-storefront-qr-download';

/**
 * StorefrontShareCard - the Overview rail's share block, India-first: WhatsApp
 * is the dominant action (big button + helper), the QR is secondary (small,
 * tap to enlarge) with copy-link + a high-res QR download exposed inline. The
 * full dialog (tap the QR) carries the enlarged QR + native share. Copy-link is
 * NOT the page's only copy - the header owns the canonical one; this is the
 * share context.
 *
 * The absolute URL is built from the live origin (client-only) so it works on
 * any deploy host without a configured base URL. A hidden 512px canvas backs
 * the PNG download so the saved QR is crisp enough for print / packaging.
 */
export default function StorefrontShareCard({
  slug,
  name,
  onShared,
}: {
  slug: string;
  name: string;
  /** Fired when the seller acts on a share (WhatsApp / copy / native) - lets the
   *  host collapse its duplicate in-body share CTAs to a secondary state. */
  onShared?: () => void;
}) {
  const t = useTranslations('connect.storefrontAdmin.console');
