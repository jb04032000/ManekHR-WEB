'use client';
// GST Input Register report (supplier-side ITC mirror). i18n via finance.reports
// (shared common.* + gstRegisters.inputRegister.*). Cross-link: header/filters from
...
// GstInputRegisterRow has the same shape as GstOutputRegisterRow (supplier-side mirror)
// Fields: entryDate, voucherNumber, partyName, partyGstin, hsnCode,
//         taxableAmountPaise, igstPaise, cgstPaise, sgstPaise, totalGstPaise, totalPaise
//         + eligibleIgstPaise, eligibleCgstPaise, eligibleSgstPaise from API
interface GstInputRegisterRow extends GstOutputRegisterRow {
  eligibleIgstPaise?: number;
  eligibleCgstPaise?: number;
  eligibleSgstPaise?: number;
}

export default function GstInputRegisterPage({ params }: { params: Promise<{ firmId: string }> }) {
  const { firmId } = use(params);
  const t = useTranslations('finance.reports');
  const ws = useWorkspaceStore((s) => s.currentWorkspace);
  const [rows, setRows] = useState<GstInputRegisterRow[]>([]);
  ...
  const handleRun = async ({ dateFrom, dateTo }: { dateFrom: string; dateTo: string }) => {
    ...
    setState('loading');
    try {
      const data = await financeReportsApi.gstInputRegister(ws._id, firmId, dateFrom, dateTo);
      setRows(data.rows as GstInputRegisterRow[]);
  ...
  const columns = [
    ...
    {
      title: t('common.billNo'),
      key: 'inv',
      render: (_: unknown, r: GstInputRegisterRow) => (
        <DrillLink
          firmId={firmId}
    ...
    { title: t('common.supplier'), dataIndex: 'partyName', key: 'party', ellipsis: true },
    ...
    {
      title: t('gstRegisters.inputRegister.eligibleIgst'),
      dataIndex: 'eligibleIgstPaise',
      key: 'eligIgst',
      align: 'right' as const,
      render: (v: number | undefined) => (v != null && v > 0 ? fmtPaise(v) : '-'),
    },
    {
      title: t('gstRegisters.inputRegister.eligibleCgst'),
      dataIndex: 'eligibleCgstPaise',
      key: 'eligCgst',
      align: 'right' as const,
      render: (v: number | undefined) => (v != null && v > 0 ? fmtPaise(v) : '-'),
    },
    {
      title: t('gstRegisters.inputRegister.eligibleSgst'),
      dataIndex: 'eligibleSgstPaise',
      key: 'eligSgst',
      align: 'right' as const,
      render: (v: number | undefined) => (v != null && v > 0 ? fmtPaise(v) : '-'),
    },
  ...
  return (
    <div>
      <ReportToolbar
        firmId={firmId}
        reportName={t('gstRegisters.inputRegister.title')}
        category={t('gstRegisters.inputRegister.category')}
      ...
      <div style={{ padding: 24 }}>
        ...
        {state === 'success' && (
          <>
            ...
            <ReportTable<GstInputRegisterRow>
              dataSource={rows}
              columns={columns}
        ...
        {state === 'error' && <Alert type="error" title={t('gstRegisters.inputRegister.error')} />}
      </div>
    </div>
  );
}
