...
/**
 * Loading UI for `/connect/services` (the Services browse, Slice B3). Mirrors
 * MarketplaceBrowseScreen section-for-section (the services surface reuses that
 * screen): the gold hero (title + subtitle + cross-link + search band), the
 * service-type category strip, then the 2-column body (filter rail + grid), and
 * the right safety rail. Plain server markup (no client components); root
 * aria-hidden. Keep in sync with app/connect/marketplace/loading.tsx +
...
/** One result card placeholder: cover image + category eyebrow + 2-line title +
  ...
  return (
    ...
    >
      <main className="min-w-0 flex-1">
        {/* Hero: gold band with title + subtitle + cross-link + the search band. */}
        <header
        ...
        {/* Service-type strip (horizontally-scrollable chips). */}
        ...
        {/* Body: filter rail + results grid. grid-cols-1 base keeps the mobile
        ...
        <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
          ...
          {/* Results: toolbar (count + sort) + grid. */}
