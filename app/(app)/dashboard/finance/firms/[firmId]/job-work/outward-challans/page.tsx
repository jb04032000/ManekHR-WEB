'use client';
// Finance polish (job-work): i18n via finance.jobWork.outward + .listCommon; DsPageHeader
...
import { listJwOutwardChallans } from '@/lib/actions/finance/job-work.actions';
import { listParties } from '@/lib/actions/finance.actions';
import type { JobWorkOutwardChallan, Party } from '@/types';
...
export default function OutwardChallansPage() {
  ...
  const [statusTab, setStatusTab] = usePersistedState<string>(
    `finance:jobWork:outward:status:${firmId}`,
    'all',
  );
  ...
  useEffect(() => {
    ...
    listJwOutwardChallans(wsId, firmId, {
  ...
  const columns: ColumnsType<JobWorkOutwardChallan> = [
    {
    ...
    {
      title: t('outward.colReturnLines'),
      dataIndex: 'returnLines',
      render: (lines: unknown[]) => (
        <span style={{ fontSize: 14 }}>{Array.isArray(lines) ? lines.length : 0}</span>
      ),
    },
    {
      title: t('outward.colLinkedInvoice'),
      dataIndex: 'jwInvoiceId',
      render: (v: string | undefined, row: JobWorkOutwardChallan) =>
        v ? (
          <Button
            type="link"
            size="small"
            onClick={(e) => {
              e.stopPropagation();
              router.push(`/dashboard/finance/firms/${row.firmId}/job-work/invoices/${v}`);
            }}
          >
            {t('outward.viewInvoice')}
          </Button>
        ) : (
          <span style={{ color: 'var(--cr-text-3)', fontSize: 13 }}>-</span>
        ),
    },
    {
    ...
    },
    {
      ...
      render: (_v, row) => (
        <Button
          ...
          icon={<EyeOutlined />}
          onClick={() =>
            router.push(`/dashboard/finance/firms/${firmId}/job-work/outward-challans/${row._id}`)
          }
        />
      ),
    },
  ...
  return (
    <div className="p-6">
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 8, marginBottom: 8 }}>
          ...
          <DatePicker.RangePicker
            aria-label={t('listCommon.dateRange')}
            value={dateRange}
      ...
      {error ? (
        ...
        <Empty
          image={Empty.PRESENTED_IMAGE_SIMPLE}
          description={t('outward.noMatch', { query: search.trim() })}
        />
      ) : (
        <DsTable<JobWorkOutwardChallan>
          ...
          onRow={(row) => ({
            ...
            onClick: () =>
              router.push(
                `/dashboard/finance/firms/${firmId}/job-work/outward-challans/${row._id}`,
              ),
          })}
        />
      )}
