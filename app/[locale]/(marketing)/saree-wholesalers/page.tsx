...
export async function generateMetadata({
  ...
  const t = await getTranslations('marketing.pages.sareeWholesalers.meta');
  return {
    title: t('title'),
    description: t('description'),
    alternates: marketingAlternates('/saree-wholesalers', locale),
    openGraph: { title: t('title'), description: t('description'), url: '/saree-wholesalers' },
  };
}

// SEO landing for "saree wholesalers / Surat saree wholesale" intent. Reuses the
// marketplace FAQ (already localized) for the body and adds its own meta + hero +
// intro block (marketing.pages.sareeWholesalers.*). Mirrors the structure of
// app/(marketing)/textile-marketplace/page.tsx.
export default async function SareeWholesalersPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('marketing.pages.sareeWholesalers');
  const tFaq = await getTranslations('marketing.pages.marketplace.faq');
  // FAQPage JSON-LD mirrors the visible FAQ; forced to English (schema never localized).
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: 'Saree Wholesalers', path: '/saree-wholesalers' },
        ])}
      />
