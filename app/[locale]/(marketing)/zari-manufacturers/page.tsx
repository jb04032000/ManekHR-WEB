...
export async function generateMetadata({
  ...
  const t = await getTranslations('marketing.pages.zariManufacturers.meta');
  return {
    title: t('title'),
    description: t('description'),
    alternates: marketingAlternates('/zari-manufacturers', locale),
    openGraph: { title: t('title'), description: t('description'), url: '/zari-manufacturers' },
  };
}

// SEO landing for "zari manufacturers / embroidery units" intent. Reuses the
// marketplace FAQ (already localized) and its own meta + hero + intro block
// (marketing.pages.zariManufacturers.*). Mirrors saree-wholesalers/page.tsx.
export default async function ZariManufacturersPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('marketing.pages.zariManufacturers');
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: 'Zari Manufacturers', path: '/zari-manufacturers' },
        ])}
      />
