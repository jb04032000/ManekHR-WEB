...
export default function DeliveryChallansPage({ params }: { params: Promise<{ firmId: string }> }) {
  ...
  const [state, setState] = usePersistedState<string[]>(
    `finance:sales:deliveryChallans:status:${firmId}`,
    [],
  );
  ...
  const CHALLAN_TYPE_LABELS: Record<string, string> = {
    goods: t('deliveryChallans.types.goods'),
    job_work: t('deliveryChallans.types.jobWork'),
    sample: t('deliveryChallans.types.sample'),
    returnable: t('deliveryChallans.types.returnable'),
  };
  ...
  const columns = useMemo(
    () => [
      {
        ...
        width: 140,
        render: (v: string, row: DeliveryChallan) => (
          <Link
            href={`/dashboard/finance/firms/${firmId}/sales/delivery-challans/${row._id}`}
            className="font-bold"
      ...
      },
      {
        ...
        width: 100,
        sorter: (a: DeliveryChallan, b: DeliveryChallan) =>
      ...
      {
        title: t('listCommon.col.challanType'),
        dataIndex: 'challanType',
        width: 120,
        render: (val?: string) => (val ? (CHALLAN_TYPE_LABELS[val] ?? val) : '-'),
      },
      {
    ...
    [firmId, t, CHALLAN_TYPE_LABELS],
  );

  const bulkActions: BulkAction[] = [
    {
      key: 'convert',
      label: t('deliveryChallans.convertToInvoice'),
  ...
  return (
    <div>
      <DsPageHeader
        ...
        titleAside={<InfoTooltip text={t('deliveryChallans.explainer')} />}
        style={{ marginBottom: 16 }}
        right={
          <Link href={`/dashboard/finance/firms/${firmId}/sales/delivery-challans/new`}>
            <DsButton dsVariant="primary" icon={<PlusOutlined />}>
              {t('deliveryChallans.new')}
            </DsButton>
          </Link>
        }
      />
      ...
      {error ? (
        ...
        <div className="py-16 text-center">
          <div className="mb-2 text-base font-bold">{t('deliveryChallans.emptyTitle')}</div>
          <div className="mb-4 text-sm" style={{ color: 'var(--cr-text-3)' }}>
            {t('deliveryChallans.emptyBody')}
          </div>
          <Link href={`/dashboard/finance/firms/${firmId}/sales/delivery-challans/new`}>
            <DsButton dsVariant="primary">{t('deliveryChallans.new')}</DsButton>
          </Link>
        </div>
      ) : (
        <DsTable
