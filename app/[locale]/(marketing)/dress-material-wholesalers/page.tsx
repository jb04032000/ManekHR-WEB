...
export async function generateMetadata({
  ...
  const t = await getTranslations('marketing.pages.dressMaterialWholesalers.meta');
  return {
    title: t('title'),
    description: t('description'),
    alternates: marketingAlternates('/dress-material-wholesalers', locale),
    openGraph: {
      title: t('title'),
      description: t('description'),
      url: '/dress-material-wholesalers',
    },
  };
}

// SEO landing for "dress material wholesalers" intent. Reuses the marketplace FAQ
// (already localized) and its own meta + hero + intro block
// (marketing.pages.dressMaterialWholesalers.*). Mirrors saree-wholesalers/page.tsx.
export default async function DressMaterialWholesalersPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('marketing.pages.dressMaterialWholesalers');
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: 'Dress Material Wholesalers', path: '/dress-material-wholesalers' },
        ])}
      />
      ...
      <section className="bg-white py-16 sm:py-20 lg:py-24">
        <Container>
          <SectionHeading
            id="dress-material-wholesalers-faq"
