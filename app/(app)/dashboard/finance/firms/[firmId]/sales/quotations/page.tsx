...
export default function QuotationsPage({ params }: { params: Promise<{ firmId: string }> }) {
  ...
  const [state, setState] = usePersistedState<string[]>(
    `finance:sales:quotations:status:${firmId}`,
    [],
  );
    ...
    () => [
      {
        ...
        render: (v: string, row: Quotation) => (
          <Link
            href={`/dashboard/finance/firms/${firmId}/sales/quotations/${row._id}`}
            className="font-bold"
      ...
      },
      {
        ...
        sorter: (a: Quotation, b: Quotation) => (a.grandTotalPaise ?? 0) - (b.grandTotalPaise ?? 0),
        render: fmt,
      },
      {
  ...
  const bulkActions: BulkAction[] = [
    {
      key: 'convert',
      label: t('quotations.convertToInvoice'),
      onClick: () => setConvertOpen(true),
    },
    {
      key: 'send',
      label: t('listCommon.action.send'),
      onClick: () => {
        /* TODO: Wave 8 bulk send */
      },
    },
  ];
  ...
  return (
    <div>
      <DsPageHeader
        ...
        right={
          <Link href={`/dashboard/finance/firms/${firmId}/sales/quotations/new`}>
      ...
      {error ? (
        ...
        <div className="py-16 text-center">
          <div className="mb-2 text-base font-bold">{t('quotations.emptyTitle')}</div>
          <div className="mb-4 text-sm" style={{ color: 'var(--cr-text-3)' }}>
            {t('quotations.emptyBody')}
          </div>
          <Link href={`/dashboard/finance/firms/${firmId}/sales/quotations/new`}>
            <DsButton dsVariant="primary">{t('quotations.new')}</DsButton>
          </Link>
        </div>
      ) : (
        <DsTable
